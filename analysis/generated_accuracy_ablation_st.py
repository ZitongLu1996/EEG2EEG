...
for i in range(10):
    if i != 8:
        for j in range(10):
            if i != j and j != 8:
                fake_linear = np.load('generated_linearmodel/st_Sub' + str(i + 1) + 'ToSub' + str(j + 1) + '_final.npy')
                fake_linear = np.average(fake_linear, axis=2)
...
np.save('Analysis_results/avgtrials_comparisons_st/3corrs_linear.npy', y_linear)"""
y_linear = np.load('Analysis_results/avgtrials_comparisons_st/3corrs_linear.npy')
...
for i in range(10):
    if i != 8:
        for j in range(10):
            if i != j and j != 8:
                fake_nocombination = np.load('generated_nocombination/st_Sub' + str(i + 1) + 'ToSub' + str(j + 1) + '_final.npy')
                fake_nocombination = np.average(fake_nocombination, axis=2)
                real = np.load('eeg_data/test/st_sub' + str(j + 1).zfill(2) + '.npy')
                real = np.average(real, axis=2)
...
np.save('Analysis_results/avgtrials_comparisons_st/3corrs_nocombination.npy', y_nocombination)"""
y_nocombination = np.load('Analysis_results/avgtrials_comparisons_st/3corrs_nocombination.npy')
...
for i in range(10):
    if i != 8:
        for j in range(10):
            if i != j and j != 8:
                fake_nocosineloss = np.load('generated_nocosineloss/st_Sub' + str(i + 1) + 'ToSub' + str(j + 1) + '_final.npy')
                fake_nocosineloss = np.average(fake_nocosineloss, axis=2)
                real = np.load('eeg_data/test/st_sub' + str(j + 1).zfill(2) + '.npy')
                real = np.average(real, axis=2)
...
np.save('Analysis_results/avgtrials_comparisons_st/3corrs_nocosineloss.npy', y_nocosineloss)"""
y_nocosineloss = np.load('Analysis_results/avgtrials_comparisons_st/3corrs_nocosineloss.npy')
...
ax.bar(x+0.5*w, y_nocosineloss_avg, color='white', edgecolor='deepskyblue', yerr=y_nocosineloss_err, width=w, hatch='///', capsize=6, ecolor='deepskyblue', zorder=2, label='Non-CosineLoss')
