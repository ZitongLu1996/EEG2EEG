    ...
    if i != 8:
        ...
        for j in range(10):
            if i != j and j != 8:
                ...
                fake = np.average(fake, axis=(0, 1, 2))
                ...
                axs[index1, index2].plot(t, fake, label='fake')
                axs[index1, index2].plot(t, real, label='real')
                axs[index1, index2].set_title('To Sub' + str(j+1), fontsize=14)

                index = index + 1
        plt.legend()
        plt.savefig('Analysis_results/avgtrials_comparisons_st/FromSub' + str(i+1) + '.jpg', dpi=200)
